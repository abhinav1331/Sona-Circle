...
<div class="row feeds_section">
 <div class="col-lg-6">
     <div class="feed_d">
          <h1>Feed Description</h1><p><?php echo $feedDetails[0]->description ?></p>
     </div>
</div>
 <div class="col-lg-6">
     <div class="feed_img">
     <h1>Feed Image</h1><?php if($feedDetails[0]->image_url != "") { ?><img src="<?php echo $feedDetails[0]->image_url; ?>" width="200px" height="200px" alt=""><?php } ?>
    </div>
     </div>
 </div>
</div>
...
<ul  class="nav nav-pills">
        ...
        <a  href="#1a" data-toggle="tab">Feeds Liked</a>
			...
			<li><a href="#3a" data-toggle="tab">Spam Feeds</a>
			</li>
		</ul>
			  ...
			  <div class="tab-pane active" id="1a">
      				<table id="myTable" class="table-striped">
						...
						<tbody>
							<?php 
							$i=1;
							foreach( $wpdb->get_results("SELECT * FROM `im_feeds_like` WHERE `feed_id` = $feedID") as $key => $row) {
				...
				<div class="tab-pane" id="2a">
     				<table id="myTable1" class="table-striped">
						...
						<tbody>
							<?php 
							$i = 1;
							foreach( $wpdb->get_results("SELECT * FROM `im_feed_comment` WHERE `feed_id` = $feedID") as $key => $row) {
								$user_info = get_userdata($row->user_id);
								$userImageUrl = get_user_meta($row->user_id , "userImageUrl" , true);
							?>
							<tr>
        ...
        <div class="tab-pane" id="3a">
          			<table id="myTable2" class="table-striped">
						...
						<tbody>
							<?php 
							$i = 1;
							foreach( $wpdb->get_results("SELECT * FROM `Table: im_feeds_spam` WHERE `feed_id` = $feedID") as $key => $row) {
								$user_info = get_userdata($row->user_id);
								$userImageUrl = get_user_meta($row->user_id , "userImageUrl" , true);
							?>
							<tr>
