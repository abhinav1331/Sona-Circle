	...
	<div class="bx-innr">
		...
		<table id="myTable" class="table-striped">
			...
			<tbody>
				<?php 
				$i=1;
				foreach( $wpdb->get_results("SELECT * FROM `im_discussion` WHERE `status` = 1 ORDER BY `id` DESC") as $key => $row) {
					$user_info = get_userdata($row->user_id);
					$myCat = $wpdb->get_results("SELECT * FROM `im_discussion_rel` WHERE `dis_id` = $row->id");
					$myArrayCat = array();
					foreach ($myCat as $key => $value) {
						$catID = $value->cat_id;
						$myCat = $wpdb->get_results("SELECT * FROM `im_discussion_category` WHERE `id` = $catID");
						$myArrayCat[] = $myCat[0]->name;
					}
				?>
				<tr>
					<td><?php echo $i; ?></td>
					...
					<td><p><?php echo implode(" , " , $myArrayCat); ?></p></td>
