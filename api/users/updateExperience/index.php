	...
	if(empty($user)) {
		...
		if($rowCheck != 0) {
			...
			$endDatee = $endDateA[2]."-".$endDateA[1]."-".$endDateA[0];
				...
				$query1="UPDATE `im_experiance` SET `title` = '$title' WHERE `id`= $id";
				$wpdb->query($query1);	
				$query2="UPDATE `im_experiance` SET `start_date` = '$startDate' WHERE `id`= $id";
				$wpdb->query($query2);	
				$query3="UPDATE `im_experiance` SET `end_date` = '$endDatee' WHERE `id`= $id";
				$wpdb->query($query3);	
				$query4="UPDATE `im_experiance` SET `is_present` = '$isPresent' WHERE `id`= $id";
				$wpdb->query($query4);	
