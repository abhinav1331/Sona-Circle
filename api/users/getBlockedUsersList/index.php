...
if($userID == "") {
	...
	if(empty($user)) {
		...
		$myContacts = $wpdb->get_results("SELECT * FROM `im_conects` WHERE `user_id` = $userID AND `status` = 2");
		if(empty($myContacts)) {
			...
			foreach ($myContacts as $key => $value) {
				...
				$myArray[] = array("friendID" => $value->user_friend_id , "name" => $name , "profession" => $profession , "userImageUrl" => $userImageUrl);
			}
			if(empty($myArray)) {
				$json = array("success" => 0, "result" => array(), "error" =>  "No Data Found");
			} else {
