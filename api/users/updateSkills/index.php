...
if($userID == "" || $id == "" || $title == "") {
	...
	if(empty($user)) {
		...
		$rowCheck = $wpdb->get_var("SELECT count(`id`) FROM `im_skills` WHERE `id` = $id");
		if($rowCheck != 0) {
			$rowData = $wpdb->get_results("SELECT * FROM `im_skills` WHERE `id` = $id");
			$getUserID = $rowData[0]->user_id;
			if($getUserID == $userID) {
				$query1="UPDATE `im_skills` SET `name` = '$title' WHERE `id`= $id";
				$wpdb->query($query1);	
