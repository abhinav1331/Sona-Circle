...
if($userID == "") {
	...
	if(empty($user)) {
		...
		$userResultGET = array();
		$userResult = $wpdb->get_results("SELECT * FROM `im_education` WHERE `user_id` = $userID ORDER BY `id` DESC");
		foreach ($userResult as $key => $value) {
			...
			$userResultGET[] = array("id" => $value->id , "title" => $value->title ,"course" => $value->course_name ,  "startDate" => date("d/m/Y" , strtotime($value->start_date)) , "endDate" => $endData);
		}
