...
if($userID == ""|| $objID == "") {
	...
	if(empty($user)) {
		...
		$myTable = $wpdb->get_results("SELECT * FROM `im_feed_comment` WHERE `id` = $objID");
		if(empty($myTable)) {
			$json = array("success" => 0, "result" => 0, "error" =>  "Invalid Comment");
		} else {
			
			$feed_id = $myTable[0]->feed_id;
			$im_feeds = $wpdb->get_results("SELECT * FROM `im_feeds` WHERE `id` = $feed_id");
			$authorID = $myTable[0]->user_id;
			$feedAuthor = $im_feeds[0]->user_id;
			if($authorID == $userID || $feedAuthor == $userID) {
				$wpdb->query("DELETE FROM `im_feed_comment` WHERE `id` = $objID");
